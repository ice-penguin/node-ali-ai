...
function sortKey(info){
	...
	for (var i = 0; i < keyArr.length; i++) {
		...
		str += keyArr[i]+"="+percentEncode(info[keyArr[i]].toString());
		readyStr += percentEncode(keyArr[i])+"="+percentEncode(info[keyArr[i]].toString());
	}
...
var getRequryStr = function(options,body){
	...
	var str = sortKey(obj).str;
	readyStr = options.method + "&" + percentEncode("/") + "&" + percentEncode(obj.readyStr);
...
var percentEncode = function(str){
	...
	for (var i in str) {
		//判断是否需要编码
		result += (isNormal.test(str[i])?str[i]:("%"+str[i].charCodeAt().toString(16).toUpperCase()))
		// console.log(str[i].charCodeAt(),str[i].charCodeAt().toString(16));
		// if(str[i] == "中" || str[i] == "文"){
		// 	console.log(str[i].charCodeAt());
		// 	console.log(isNormal.test(str[i])?str[i]:("%"+str[i].charCodeAt().toString(16).toUpperCase()));
		// }
